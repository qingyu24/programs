...
public class TestUser_HLD {
	...
	public static ArrayList<TestUser_HLD> list;
	
	private static TestUser_HLD _tu;
	public static TestUser_HLD getSingle(){
		if(_tu == null){
			_tu = new TestUser_HLD();
		}
		return _tu;
	}

	public ArrayList<TestUser_HLD> getlist() {
		if (list != null) {
			return list;
		} else {
			list = new ArrayList<TestUser_HLD>();
			list.add(new TestUser_HLD());
			list.add(new TestUser_HLD());
			list.add(new TestUser_HLD());
			list.add(new TestUser_HLD());
			return list;
	...
	public TestUser_HLD set(RoomUser m_user) {
		this.reset();
	...
	public TestUser_HLD set(long m_roleId, Boolean m_piao, int m_mg, int m_ag) {
		this.reset();
	...
	public void sumGang(ArrayList<TestUser_HLD> list) {
		int sanren = 3;
		...
		// 其他的玩家都要-1 * count;
		Iterator<TestUser_HLD> it = list.iterator();
		while (it.hasNext()) {
			TestUser_HLD ru = it.next();
